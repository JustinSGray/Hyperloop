...
pod at a relative velocity equal to the pod speed and as the air is forced through the smaller bypass area, it increases in speed. Assuming a circular cross section
for both the pod and tube, then the bypass air must travel through an area given by

\nomenclature{A}{Area}
\nomenclature{r_{tube}}{Tube Radius}
\nomenclature{r_{pod}}{Pod Radius}
\begin{equation*}
A_{bypass} = \pi(r_{tube}^2-r_{pod}^2)
\end{equation*}
\nomenclature{\rho_{air}}{Air Density}
\nomenclature{\dot{W}_{bypass}}{Bypass Mass Flow Rate (kg/s)}
...
\nomenclature{V_{pod}}{Pod Velocity}
\begin{equation*}
...
\nomenclature{C_{p}}{Heat capacity at constant pressure}
\begin{equation*}
...
\nomenclature{{\theta}_{nni}}{Non-normal Incidence factor}
...
\nomenclature{P_{rad}}{Radiated Power}
\nomenclature{L}{Length}
...
\nomenclature{g}{Acceleration of gravity}
\nomenclature{\beta}{Volume coefficient of expansion (K)}
\nomenclature{\upsilon}{Kinematic Viscosity}
\nomenclature{Pr}{Prandtl Number}
\nomenclature{Gr}{Grashof Number}
\nomenclature{Ra}{Rayleigh Number}
