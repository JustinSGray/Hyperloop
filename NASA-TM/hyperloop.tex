...
is a function of the maximum pod travel Mach number and the first stage compressor pressure ratio. In Fig~\ref{f:compressorXDSM} this relationship 
...
\begin{table}
  ...
  \begin{tabular}{l c c}
    ...
    $A_{pod}$ ($m^2$)   &        1.4       &     4.0   \\ 
    Area Ratio          &        2.76      &     2.14  \\
    Max Travel Mach     &        .95       &     .95   \\
    \hline
  \end{tabular}
\end{table}

In the original work it was stated that the tube dimensions were selected to keep the speed of the air bypassing around 
the pod below Mach 1.0. Air traveling at Mach 1.0 through a confined space has a unique property: it moves with the
maximum amount of mass flow per unit area possible. In other words, it is physically impossible to move more air through 
a tube than can be moved when the air travels at Mach 1.0. This situation, called choked flow, is the motivation for limiting 
the Mach number of the bypass air. In this section we show why this phenomenon occurs and how it results in a coupling between 
the size of the tube and the size of the pod, so that it is not possible to have both as independent design variables. 

Let us start by assuming that the pod is sealed and does not have a compression system. 
This notional situation is depicted in Fig.~\ref{f:ClosedPod}. Here $A_{bypass}$ is obviously the 
smallest area that the flow must pass through, so the amount of choked flow that can pass through 
this area represents the theoretical maximum possible. 
...
As a hyperloop pod travels down the tube, air moves toward it (or it moves toward the air) with a relative velocity equal to its
own speed. The creates an effective mass flow rate of air around the pod which is a function of the travel speed of the pod.  

\begin{equation}
\dot{W} = \rho_{air} A_{tube} V_{pod}
\label{e:w-tube}
\end{equation}

Thus the travel speed is limited by the situation where the relative mass flow rate, $\dot{W}$, is 
equal to the choked flow limit imposed by a given $A_bypass$. By applying isentropic flow relationships, 
we can find the required area ratio between $A_{tube}$ and $A_{bypass}$. 
...
It is more natural to think in terms of the ratio between $A_{tube}$ and $A_{pod}$, so eqn.~\ref{e:a-over-astar} can be converted 
into those quantities as follows. 

\begin{equation}
\frac{A_{tube}}{A_{pod}} = \frac{1}{\left(\frac{\gamma+1}{2}\right)^{-\frac{\gamma+1}{2\left(\gamma-1\right)}}\frac{\left(1+\frac{\gamma-1}{2}MN^{2}\right)^{\frac{\gamma+1}{2\left(\gamma-1\right)}}}{MN} - 1}
\label{e:atube-to-apod}
\end{equation}

By setting Mach number to be 1.0 in eqn.~\ref{e:atube-to-apod}, we can compute the area ratio that will yield choked bypass flow. Call 
the area ratio where this occurs, $\frac{A^*_{tube}}{A_{pod}}$. Any area ratio smaller than this (i.e. $A_{pod}$ grows relative to $A_{tube}$) 
would result in the required mass flow rate being greater than the allowable mass flow rate, hence some amount of air would get 
trapped in front of the pod and dragged along with it through the tube. In this situation, the pod acts somewhat like a piston, 
causing a build up of pressure in front of it. To avoid this, for any given pod size we must pick a suitable tube radius such that 
the following constraint is satisfied. 

\begin{equation}
\frac{A_{tube}}{A_{pod}} \ge \frac{A^*_{tube}}{A_{pod}}
\label{e:tube-size-constraint}
\end{equation}

Given a certain size for the pod and its maximum travel speed we can now compute the required size of the tube via 
equations \ref{e:w-tube}, \ref{e:atube-to-apod}, and \ref{e:tube-size-constraint}. In other words the size of the pod and the size of the tube and 
the pod travel mach number are all coupled together. Specify any two of these quantities and the third will be determined. 
For example if you took the pod size and the tube size as design variables, this would then limit the 
achievable travel speed of your design. Fig.~\ref{f:flowLIMIT} illustrates this situation for range of $\frac{A_{tube}}{A_{pod}}$ values.
This graph compares the choked flow limit (dashed lines) with the actual required mass flow rate (solid lines) given an area ratio and pod travel Mach number. 
At lower speeds the dashed lines sit above their respective solid lines, indicating some flow rate margin. At some speed, dependent on the 
area ratio, the dashed and solid lines intersect indicating the maximum travel Mach number achievable. 

Note that at the area ratios from the baseline design, around 2-3, the maximum travel speed for a sealed design 
without any compression system would be around Mach .35. So for this notional concept of a sealed passenger pod, 
to reach the transonic speeds specified, the area ratio would need to be increased dramatically. 
This is where the on-board compression system comes into play, by providing a means of compressing some of the air and 
moving it through the passenger pod so that it does not need to go around it. Essentially, the compression systems job
in this regard, is to provide a means of getting around the choked flow limit without growing the size of the tube so 
dramatically. Having established the fundamental coupling between pod size, tube size, and travel speed, we no move on to 
consider this more complex scenario. 
