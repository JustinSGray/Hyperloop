...
$M_{pod}$ and $M_{bypass}$ that defines an area ratio, $\frac{A_{bypass}}{A_{tube}}$.
...
Figure \ref{f:choked-flow} show the effect Eq.~(\ref{e:a-over-astar}) over a range of values for $M_{pod}$ and $M_{bypass}$. 
...
choked flow limit. Therefore, the fastest portion of the trip, illustrated in red, is modified
by as the simulation runs and re-computes pod speed.
