...
limit (dashed lines) and the required flow (solid lines) of figure \ref{f:flowLIMIT} signifies the compressor flow rates necessary to achieve a given speed. 
...
charted rather than an area ratio. Although it isn't shown, the pod size is also free to vary at each optimized point. Somewhat surprisingly, the pod size remains
...
increasing the inlet diameter harms the bypass area at a greater rate than it improves compressor intake. Unlike figure \ref{f:flowLIMIT}, this plot also assumes
the capsule contains a compression system and is therefore shifted further right than if the intersection points from figure \ref{f:flowLIMIT} were directly
overlayed. 
...
with $Qmax=\left(T_{hot,in} − T_{cold,in}\right) [ \dot{m} C_{p} ]_{fluid}$ where the product $\dot{m} C_{p}$ of whichever fluid is lowest is used.
...
method. The effectiveness for a counter flow heat exchanger with a $\frac{C_{min}}{C_{max}}$ of ~0.25 was chosen. The following conditions satisfied an
energy balance with an extremely optimistically assumed effectiveness of 0.9765, and the proposed requirement to cool the
