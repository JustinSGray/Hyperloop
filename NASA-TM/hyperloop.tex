...
exchangers. A simple quasi-steady equilibrium heat balance
...
and cp is the specific heat of air given by the relations in Ref. XX. 
...
proposed in Ref. YY, and reproduced in many foundational heat transfer textbooks:
...
The correlation is applicable for estimating free convection from long, horizontal cylinders.
Properties for air ($h$, $k$, $\beta$, $\nu$ and $\alpha$) are based on the film temperature
of the thermal boundary layer and are given by the regressions in Ref. XX.
...
around the model’s nominal values as listed in Table XX.


ttabvalsdjasd

A histogram and a normal distribution of tube temperature
based on 10,000 samples are shown in Fig. XX.
The mean tube temperature is 105\degF with a standard deviation of 10\degF.
Exceeding 130\degF is estimated to occur only 0.5 percent of the time.
