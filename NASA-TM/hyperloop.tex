...
system cost. It's worth noting that the pod size is also free to vary at each optimized point and is plotted on the green line. Somewhat surprisingly, the pod size remains
...
The compression system serves dual purposes. It provide a means of exceeding the nominal Kantrowitz limit as explored in the previous section and also supplies the resulting
pressurized air to support the air bearing system.

Each of these functions requires the compressors to move a specified amount of air to a specific pressure gain, which combine to define the total airflow for the whole
...
The compression systems is modeled as a one dimensional cycle, representing components as a thermodynamic processes that are subsequently chained together. 
Each component is responsible for calculating gas properties across its boundaries, appropriately enforcing conservation equations across the entire system. 
The subsystem leverages the open-source Cantera package, which is responsible for calculating chemical kinetics, thermodynamics, and transport processes.
These models predict the instantaneous power consumption of the compressors and determine the upstream conditions necessary to supply sufficient airflow to the bearings.
