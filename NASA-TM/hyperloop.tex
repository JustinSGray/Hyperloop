...
shows up as a feedback connection between the performance and the second compressor subsystems. 
...
must become relative to the tube area, leaving less area for the pod. For $M_{pod}$ equal to 1, $\frac{A_{bypass}}{A_{tube}}$ must 
equal 1 as well, forcing $A_{pod}$ to be 0. So it is physically impossible for a closed pod to travel through a 
tube with $M_{pod}$ at the speed of sound. Note that the values from Table \ref{t:hyperbase} gives area ratios around .6, 
which yields a maximum $M_{pod}$ of about .3-.4 for a closed pod. 
...
at $M_{pod}$. The challenge is that the pod cannot be perfectly hollow. It must provide some space for the passengers
...
This increase is strongly dependent on the amount of "blockage" assumed and slightly dependent on the compressor face Mach number.
...
The system, shown in Fig. \ref{f:compressorXDSM}, is modeled as a one-dimensional cycle,
...
%\label{f:comp}
%\end{figure}
The simulation uses PyCycle, an open-source cycle analysis tool that employs Cantera \cite{goodwin2009cantera} for thermodynamic calculations.
The design of PyCycle was heavily influenced by NPSS;
further information on these tools can be found in the appendix and online documentation. \cite{Lytle}
...
The notional velocity profile described in the original proposal, and shown in Fig. \ref{f:velocity},
...
choked flow limit. Therefore, the fastest portion of the trip is modified
by the simulation at run-time, as illustrated in the red in Fig. \ref{f:velocity}.
...
The integrated area under the curve stays constant, since it represents the total length of the tube.
The speed constraints outlined in this paper increase total mission time on the order of five minutes,
however this will ultimately be determined by the chosen tube diameter.
Multiplying the calculated mission time by the average compressor power consumption (plus a 30\% safety margin)
...
are also directly overlayed in red and green respectively. 
...
and aretyc based on the assertion that the compression cycle does not need to be cooled.
